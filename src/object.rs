...
impl Map {
  pub fn new<'s>(scope: &mut HandleScope<'s>) -> Local<'s, Map> {
    unsafe {
      scope.cast_local(|sd| v8__Map__New(sd.get_isolate_ptr()))
    }
    .unwrap()
  }
  ...
  pub fn get<'s>(
    ...
    unsafe {
      scope
        .cast_local(|sd| v8__Map__Get(self, sd.get_current_context(), &*key))
    }
  }
  ...
  pub fn has<'s>(
    ...
    unsafe { v8__Map__Has(self, &*scope.get_current_context(), &*key) }
      .into()
  }
